...
(LOOP)
// continuously check keyboard input until a key is pressed
@KBD
D=M
@LOOP
D;JEQ
...
// check if key is still pressed, if not go to WHITE
@KBD
D=M
@WHITE
D;JEQ
// otherwise keep screen black
@LOOP
0;JMP
...
// check if key is still released, if not go to BLACK
@KBD
D=M
@BLACK
D;JNE
// otherwise keep screen white
@LOOP
0;JMP
